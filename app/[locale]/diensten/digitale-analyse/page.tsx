...
import {BarChart2, CheckCircle2, ChevronDown} from 'lucide-react';
...
function serviceJsonLd(locale: string) {
  ...
  return {
    ...
    '@graph': [
      {
        '@type': 'Service',
        name: isNl ? 'Digitale Analyse & Rapportage' : 'Digital Analytics & Reporting',
...
export default async function DigitaleAnalysePage({
  params,
}: {
  params: Promise<{locale: string}>;
}) {
            ...
            <AnimatedSection direction="right" delay={0.15}>
              <div className="bg-primary rounded-sm p-10 border-l-4 border-gold">
                <div className="w-14 h-14 rounded-sm bg-gold/10 flex items-center justify-center mb-6">
                  <BarChart2 size={26} className="text-gold" />
                </div>
